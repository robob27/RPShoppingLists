...
--                    if self.joyselection >= #self.itemslist then self.joyselection = 0; end
               ...
               -- print("trace:i");
                ...
                local itemName = item:getName();
                if count == 1 then
                    -- ShoppingList management - Checking if the item is in one of the shopping lists in current inventory and set the highlight colour.
                    local SLHighlightColorRed = -1.0
                    local SLHighlightColorGreen = -1.0
                    local SLHighlightColorBlue = -1.0
                    local SLNumberToFetch = 0
                    local itemNameLC = string.lower(itemName)

                    local SLInv = player:getInventory();
                    local SLItems = SLInv:getItems();
                    if(SLItems ~= nil) then
                        for SLItemIndex=0, SLItems:size() - 1 do
                            local SLItem = SLItems:get(SLItemIndex);
                            if SLItem:getType() == "RPShoppingList" then
                                local SLModData = SLItem:getModData();
                                if(SLModData.highlightColorA) and (SLModData.rpsltags) then
                                    if (SLModData.highlightColorA > 0.0)  then
                                        for SLEntry, SLQuantity in pairs(SLModData.rpsltags) do
                                            local SLEntryLC = string.lower(SLEntry)
                                            local SLContain = ""
                                            string.gsub(SLEntryLC, "*(.-)*", function(a)  SLContain = a end)
                                            
                                            if ((SLContain == "" and (SLEntryLC == itemNameLC)) or
                                                (SLContain ~= "" and string.find(itemNameLC, SLContain) ~= nil)) then
                                                SLNumberToFetch = SLNumberToFetch + SLQuantity
                                                if(SLHighlightColorRed < 0.0) then
                                                    if(SLModData.highlightColorR) then
                                                        SLHighlightColorRed = SLModData.highlightColorR;
                                                        SLHighlightColorGreen = SLModData.highlightColorG;
                                                        SLHighlightColorBlue = SLModData.highlightColorB;
                                                    else    -- Old shopping lists created before the update -> fallback to yellow.
                                                        SLHighlightColorRed = 1.0;
                                                        SLHighlightColorGreen = 1.0;
                                                        SLHighlightColorBlue = 0.0;
                                                    end
                                                end
                                            end
                                        end
                    ...
                    if SLHighlightColorRed > -1.0 then
                        local quantity = ""
                        if v.count > 2 then
                            quantity = " ("..(v.count-1)..") ";
                        end
                        self:drawText(itemName .. quantity .. " ["..(SLNumberToFetch).."]", self.column2+8+xoff, (y*self.itemHgt)+16+textDY+yoff, SLHighlightColorRed, SLHighlightColorGreen, SLHighlightColorBlue, 1.0, self.font);
                    else
                    -- ShoppingList management - section end ("if" statement will be closed later)
                    ...
                    -- ShoppingList management - "if" statement closure
                    end
                    -- ShoppingList management - section end
                end
               -- print("trace:j");
